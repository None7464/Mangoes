...
public class Autoplay {
    ...
    // =========================================================================
    //  SCANNER — THE HOT LOOP
    //
    //  2025-01-15 — MAJOR REWRITE. Old version was leaking ~40 MB/min.
    //
    //  The root cause: createScreenCapture() allocates a new BufferedImage backed
    //  by a native pixel buffer every single call. The JVM sees the Java object
    //  get garbage collected, but the *native* memory underneath isn't released
    //  until the finalizer runs — and on a busy scanner thread running 300+ FPS,
    //  the GC can't keep up. After about 10 minutes the process was sitting at
    //  600+ MB with GC pressure tanking FPS.
    //
    //  Fix: call img.flush() on the previous frame before allocating the next one.
    //  flush() releases the native surface immediately rather than waiting for GC.
    //  Memory usage dropped from growing-forever to a flat ~80 MB.
    //
    //  Old broken code that caused the leak:
    // -------------------------------------------------------------------------
    //  private static void scanner() {
    //      try {
    //          Robot screenCapture = new Robot();   // bad: new Robot() per invocation
    //          while (true) {
    //              BufferedImage img = screenCapture.createScreenCapture(captureBox);
    //              // img goes out of scope here. Native buffer stays alive until
    //              // GC finalizer runs — could be seconds, could be minutes.
    //              checkNotes(img);
    //          }
    ...
    // -------------------------------------------------------------------------
    //
    //  Second issue: a new Robot() was being created inside the method every call.
    //  Robot creation is expensive (loads native libs, opens OS handles). Doing it
    //  inside a loop running hundreds of times per second burned handles fast.
    //  The static `robot` field fixed that — one instance, reused forever.
    // =========================================================================
    ...
    private static void scanner() {
        ...
        // Declared outside the loop so we can flush the previous frame before
        // allocating the next one. This is the whole memory leak fix.
        ...
        try {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (paused.get() || !isGameFocused()) {
                    Thread.sleep(100); // don't spin at full speed while idle
                    continue;
                }

                // Take a local snapshot of captureBox under the lock.
                // The EDT might be rebuilding it if the user just hit Apply.
                Rectangle box;
                synchronized (Autoplay.class) { box = captureBox; }
                if (box == null) { Thread.sleep(50); continue; }

                // THIS is the fix for the memory leak. flush() releases the native
                // pixel buffer for the previous frame before we allocate the new one.
                if (img != null) img.flush();
                img = robot.createScreenCapture(box);

                Set<Integer> keysToHold = new HashSet<>();

                // Synchronise on keyMap so we don't read a half-rebuilt map if the
                // user applied config changes mid-frame.
                synchronized (Autoplay.class) {
                    for (var entry : keyMap.entrySet()) {
                        boolean hit = HOLD_DETECTION_ENABLED
                            ? detectHoldTail(img, entry.getKey(), entry.getValue().targetColor)
                            : detectSingleNote(img, entry.getKey(), entry.getValue().targetColor);
                        if (hit) keysToHold.add(entry.getValue().keyCode);
                    }
                }

                updateKeyStates(keysToHold);

                // Divide by elapsed rather than assuming exactly 1000ms elapsed —
                // Thread.sleep can overshoot by 10-20ms which compounds over time.
                frameCount++;
            ...
            // Normal shutdown path — restore the flag and fall through to finally.
            Thread.currentThread().interrupt();
        } finally {
            // Always flush the last frame even if we exit via exception.
            // Leaving it un-flushed is a native memory leak.
            if (img != null) img.flush();
        }
    }
    ...
    //  DETECTION — HOLD TAIL
    //
    //  2024-12-08: Added after noticing the bot was releasing hold notes way too
    //  early. Single-point detection only checked the hit circle area. Once the
    //  circle scrolled past the judgement line, detection went false and the key
    //  released — even though the tail was still on screen.
    //
    //  Checks 10 evenly-spaced points along the vertical line above the hit point.
    //  If at least 3 match the lane colour, the tail is still present and we keep
    //  the key held. Threshold of 3 gives wiggle room for anti-aliased note edges
    //  without triggering on stray single pixels.
    // =========================================================================

    private static boolean detectHoldTail(BufferedImage img, Point orig, Color target) {
        // Translate absolute screen coords to relative coords within our captured region.
        ...
        for (int i = 0; i < checkPoints; i++) {
            int cy = ry - (i * (HOLD_CHECK_OFFSET / checkPoints));
            if (cy < 0) break; // walked above the top of the capture region

            // Sample a 3x3 patch at each vertical checkpoint.
            // Breaking out immediately on first match avoids overcounting —
            // we want "how many scan lines match", not "how many pixels".
            outer:
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    int x = rx + dx, y = cy + dy;
                    if (x >= 0 && x < img.getWidth() && y >= 0 && y < img.getHeight()) {
                        if (colorMatches(img.getRGB(x, y), target)) {
                            matchCount++;
                            break outer;
                        }
                    }
    ...
    //
    //  For non-hold notes. Checks a 3x3 patch at the hit circle position.
    //  Simple and fast — just needs one matching pixel in the neighbourhood.
    // =========================================================================
    ...
    private static boolean detectSingleNote(BufferedImage img, Point orig, Color target) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                int x = rx + dx, y = ry + dy;
                if (x >= 0 && x < img.getWidth() && y >= 0 && y < img.getHeight()) {
                    if (colorMatches(img.getRGB(x, y), target)) return true;
                }
            }
        }
        return false;
    ...
    //
    //  Per-channel tolerance instead of Euclidean distance. Euclidean would be
    //  more accurate but needs a sqrt and multiplication on every pixel sample.
    //  At 500 FPS with 9 lanes each checking a 3x3 grid plus 10-point tail scan,
    //  that's a lot of sqrt calls. Per-channel abs is fast and works fine for the
    //  saturated note colours osu uses.
    // =========================================================================
    ...
    //
    //  Diff model: compare what keys *should* be held against what *are* currently
    //  held, release the difference, press the new ones.
    //
    //  2025-01-15: Old version called keyPress and keyRelease every frame for every
    //  detected note regardless of current state. That caused rapid-fire press/release
    //  cycles on hold notes which osu registered as misses every time.
    //  The currentlyHeldKeys map fixed that — we only send events when state changes.
    //
    //  Old broken code:
    // -------------------------------------------------------------------------
    //  private static void pressNotes(Set<Integer> keyCodes) {
    //      for (int kc : keyCodes) {
    //          robot.keyPress(kc);    // pressing every frame even if already held
    //          Thread.sleep(10);
    //          robot.keyRelease(kc);  // immediately releasing — hold notes always broke
    //      }
    //  }
    // -------------------------------------------------------------------------
    // =========================================================================
    ...
    private static void updateKeyStates(Set<Integer> keysToHold) {
        ...
        // Release keys no longer in the "should be held" set.
        // iterator.remove() is the correct way — calling map.remove() inside
        // a for-each throws ConcurrentModificationException even on ConcurrentHashMap.
        ...
        // Press keys that should be held but aren't yet.
